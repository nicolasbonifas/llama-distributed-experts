...
struct ggml_tensor * llama_model_loader::create_tensor(struct ggml_context * ctx, const std::string & name, const std::initializer_list<int64_t> & ne, int flags) {
    ...
    // TODO: Selective expert tensor loading optimization
    // Currently expert tensors are stored in merged/concatenated form,
    // so we load all experts and handle distribution at inference time in build_moe_ffn
    // Future optimization: split merged expert tensors and only load assigned slices
